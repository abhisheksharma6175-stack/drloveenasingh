  ...
  return (
    <>
      <section className="overflow-hidden bg-[radial-gradient(circle_at_88%_15%,rgba(45,212,191,.25),transparent_28%),linear-gradient(125deg,#0f172a,#075985_58%,#0f766e)] py-20 text-white sm:py-28">
        <div className="container-shell"><motion.div {...fadeUp} className="max-w-4xl"><p className="mb-5 inline-flex rounded-full bg-white/15 px-4 py-2 text-xs font-bold tracking-[.18em]">PATIENT-CENTERED MEDICAL HOME</p><h1 className="max-w-3xl text-4xl font-bold leading-[1.08] tracking-tight sm:text-5xl">Our Commitment to <br></br> Patient-Centered Medical Home (PCMH)</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-slate-100 sm:text-xl">Delivering coordinated, accessible, high-quality healthcare through evidence-based practices and continuous improvement.</p><Link href="/contact" className="focus-ring mt-8 inline-flex items-center gap-2 rounded-xl bg-white px-6 py-3.5 font-bold text-slate-900 transition hover:bg-cyan-50">Schedule Appointment <FaArrowRight /></Link></motion.div></div>
      </section>
